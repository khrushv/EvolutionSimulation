...
namespace app
{
    class AI: Ball
    {
        ...
        private NN nn;
        public AI(Genome g, double radius, double x, double y) : base(radius, x, y)
        {
            genome = g;
            nn = new NN(inputsCount, 20, 2);
        ...
        public void move(float centerOfFoodX, float centerOfFoodY, float time, float other)
        {
            float[] inputs = new float[inputsCount];
            inputs[0] = centerOfFoodX;
            inputs[1] = centerOfFoodY;
            inputs[2] = time;
            inputs[3] = other;
